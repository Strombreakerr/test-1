    ...
    async function getData_ServerSide()
        {
            ...
            */
            ...
            const chartURLs = [];
            const response = await fetch('VRM/VRM_ServerBuildSpecificResult.csv');
    ...
    async function getData_ClientSide()
        {
            ...
            const csvColumnData3 = [];
            const chartURLs = [];
            const response = await fetch('VRM/VRM_ClientBuildSpecificResult.csv');
            const data = await response.text();
            const table=data.split('\n').slice(1);
    ...
    async function getData_ServerSideVsClientSide()
        {
            ...
            const csvColumnData1 = [];
            const chartURLs = [];
            const response = await fetch('VRM/VRM_ServerVsClientCurrentBuild.csv');
            const data = await response.text();
            const table = data.split('\n');
            table.forEach(row => {
                    const column = row.split(',');
